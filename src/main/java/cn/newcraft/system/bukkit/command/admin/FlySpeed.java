...
public class FlySpeed extends CommandManager {
    ...
    public FlySpeed() {
        ...
        this.setPermission("ncs.command.flyspeed");
    }

    @Cmd(perm = "ncs.command.flyspeed", permMessage = "§c你需要 §2MOD §c及以上的会员等级才能使用此指令！", only = CommandOnly.PLAYER)
    public void reset(CommandSender sender, String[] args){
        Player p = (Player) sender;
        p.setFlySpeed((float) 0.1);
    }

    @Cmd(arg = "<integer>", perm = "ncs.command.flyspeed", permMessage = "§c你需要 §2MOD §c及以上的会员等级才能使用此指令！", only = CommandOnly.PLAYER)
    ...
    @Cmd(arg = "<player> <integer>", perm = "ncs.command.flyspeed", permMessage = "§c你需要 §2MOD §c及以上的会员等级才能使用此指令！")
