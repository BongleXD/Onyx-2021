package net.blastmc.onyx.bukkit.support.v1_16_R3;
...
public class Hologram_1_16_R3 implements Hologram {
    ...
    public Hologram_1_16_R3(Location loc, String... lines){
        this.loc = loc;
        ...
        for (String line : lines) {
            ...
            stand.setCustomName(new ChatMessage(line));
            this.lines.add(new HoloData(line, stand));
        }
    }

    public Hologram_1_16_R3(Location loc){
        this.loc = loc;
        this.lines = Lists.newArrayList();
        this.players = Lists.newArrayList();
        this.create = Lists.newArrayList();
    ...
    public Hologram_1_16_R3(Location loc, List<String> list) {
        this.loc = loc;
        this.lines = Lists.newArrayList();
        this.players = Lists.newArrayList();
        this.create = Lists.newArrayList();
        ...
        for (String line : list) {
            EntityArmorStand stand = new EntityArmorStand(((CraftWorld) loc.getWorld()).getHandle(), newLoc.getX(), newLoc.getY(), newLoc.getZ());
            stand.setNoGravity(true);
            stand.setInvisible(true);
            stand.setSmall(true);
            stand.setCustomNameVisible(true);
        ...
        this.create.forEach(uuid -> {
            ...
            if(p != null){
                lines.stream()
                        .map(data -> data.stand)
                        .forEach(stand -> {
                            stand.setPosition(newLoc.getX(), newLoc.getY(), newLoc.getZ());
    ...
    public void show() {
        ...
        for(Iterator<UUID> it = this.players.iterator(); it.hasNext();){
            ...
            for(int i = 0; i < lines.size(); i++){
                ...
                stand.setCustomName(animMap == null || animMap.isEmpty() ? new ChatMessage(data.line) : new ChatMessage("§f读取中..."));
    ...
    public Hologram line(String value) {
        ...
        stand.setSmall(true);
        stand.setCustomNameVisible(true);
        stand.setCustomName(new ChatMessage(value));
    ...
    public Hologram line(int line, String value) {
        ...
        try {
            ...
            stand.setCustomName(new ChatMessage(value));
            data.stand = stand;
            data.line = stand.getCustomName().getText();
    ...
    private void updateMetadata(UUID uuid, String line, int i){
        ...
        stand.setCustomName(new ChatMessage(PlaceholderAPI.setPlaceholders(p, line)));
