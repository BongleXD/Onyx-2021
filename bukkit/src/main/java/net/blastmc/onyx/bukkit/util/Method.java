...
public class Method {
    ...
    public static void vanishPlayer(Player p, Player sendTo, boolean b) {
        if(b){
            ...
            Main.getNMS().changeNameTag(sendTo, p, PlaceholderAPI.setPlaceholders(p, Method.getTagData(p).getPrefix()), " §c[已隐身]", TeamAction.UPDATE, getTagPriority(p, Onyx.getPlayerProfile(p.getUniqueId())));
        }else{
            if(p != sendTo){
                sendTo.showPlayer(p);
            }
            Main.getNMS().changeNameTag(sendTo, p, PlaceholderAPI.setPlaceholders(p, Method.getTagData(p).getPrefix()), PlaceholderAPI.setPlaceholders(p, Method.getTagData(p).getSuffix()), TeamAction.UPDATE, getTagPriority(p, Onyx.getPlayerProfile(p.getUniqueId())));
        }
    }
    ...
    public static String getTagPriority(Player p, PlayerProfile prof) {
        ...
        switch (prof.getNickPrefix()) {
            case "§6[SVIP§c+§6] ":
                priority = OnyxTagData.getData("svip_plus").getPriority();
                break;
            case "§b[SVIP] ":
                priority = OnyxTagData.getData("svip").getPriority();
                break;
            case "§a[VIP§6+§a] ":
                priority = OnyxTagData.getData("vip_plus").getPriority();
                break;
            case "§a[VIP] ":
                priority = OnyxTagData.getData("vip").getPriority();
                break;
            case "§7":
                priority = OnyxTagData.getData("default").getPriority();
                break;
            default:
    ...
    public static TagData getTagData(Player p){
        ...
        for(String group : OnyxTagData.getTagGroups()){
            ...
            if(OnyxTagData.getData(group).getPerm().isEmpty()){
                if(pri < priority) {
                    tagData = OnyxTagData.getData(group);
                    priority = pri;
                }
            }else if(p.hasPermission((OnyxTagData.getData(group).getPerm()))){
                if(pri < priority) {
                    tagData = OnyxTagData.getData(group);
                    priority = pri;
                }
