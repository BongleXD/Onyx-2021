...
public class v1_16_R3 implements NMS {
    ...
    public Hologram newInstance(Location loc, List<String> list) {
        return new Hologram_1_16_R3(loc, list);
    }

    @Override
    public Hologram newInstance(Location loc, String... lines) {
        return new Hologram_1_16_R3(loc, lines);
    }

    @Override
    public Hologram newInstance(Location loc) {
        return new Hologram_1_16_R3(loc);
    }
    ...
    public void sendFootStep(Location loc) {
        PacketPlayOutWorldParticles packet = new PacketPlayOutWorldParticles(Particles.NOTE,true, (float) loc.getX(), (float) loc.getY(), (float) loc.getZ(), 0, 0, 0, 0, 1);
    ...
    public void changeNameTag(Player sendTo, Player p, String prefix, String suffix, TeamAction action, String priority) {
        ...
        team.setDisplayName(new ChatMessage(p.getName()));
    ...
    public void openBookMenu(Player p, ItemStack book) {
        ...
        try {
            EntityPlayer ep = ((CraftPlayer) p).getHandle();
            net.minecraft.server.v1_16_R3.ItemStack nmsBook = CraftItemStack.asNMSCopy(book);
            p.setItemInHand(book);
            ep.activeContainer.c();
        } catch (Exception ex) {
            ex.printStackTrace();
        } finally {
    ...
    public void changeName(Player p, String name) {
        ...
        try {
            Field bH = entityHuman.getDeclaredField("bJ");
    ...
    public void reloadPlayer(Player p) {
        Bukkit.getScheduler().runTask(Main.getInstance(), () -> {
            ...
            ep.server.getPlayerList().moveToWorld(ep, ep.getWorldServer(), false, p.getLocation(), false);
